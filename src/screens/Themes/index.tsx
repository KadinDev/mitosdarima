...
export function Themes(){
    ...
    useEffect(() => {
        if(isAutomatic){
            const interval = setInterval(() => {
                const indiceAleatorio = Math.floor(Math.random() * themes.length);
                const wordAleatory = themes[indiceAleatorio];
    ...
    return (
        <View style={styles.Container}>
            ...
            <Header title='temas' buttonGoBack />
            <Title title='Treinar com temas' />
            <TotalPalavras total={`${themes.length} temas`} />
            <Separator/>
            
            {isAutomatic &&
                <Countdown 
                    title='próxima tema em' 
                    countdown={`${countdown}`} 
                />
            }
