...
import {palavrasAleatorias} from '@utils/RandomWords';
...
export function PalavrasScreen(){
    ...
    useEffect(() => {
        if(isAutomatic){
            const interval = setInterval(() => {
                const indiceAleatorio = Math.floor(Math.random() * palavrasAleatorias.length);
                const wordAleatory = palavrasAleatorias[indiceAleatorio];
    ...
    return (
        <View style={styles.Container}>
            ...
            <Header title='Palavras' buttonGoBack />
            <Title title='Treinar com palavras Aleatórias' />
            <TotalPalavras total={`${palavrasAleatorias.length} palavras para seu treino`} />
            <Separator/>
            
            {isAutomatic &&
                <Countdown 
                    title='próxima palavra em' 
                    countdown={`${countdown}`} 
                />
            }
